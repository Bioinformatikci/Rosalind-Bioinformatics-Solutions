...
This folder contains Python scripts written while practicing bioinformatics programming, sequence analysis, and introductory omics analysis tasks.
...
| `genome_assembly_shortest_superstring.py` | Reads FASTA fragments and builds a greedy shortest-superstring assembly. |
...
| `lcsm_shared_motif_search.py` | FASTA-formatted sequence file, currently referenced as `../txt.txt` | Longest shared motif printed to standard output |
...
Run each script from the folder where its expected input files are available. Example:

```bash
python genome_assembly_shortest_superstring.py
```

Recommended Python packages for the full folder are `pandas`, `numpy`, `scipy`, `matplotlib`, and `matplotlib-venn`.

## Notes

These scripts are educational and exploratory. A future improvement would be to standardize command-line arguments and add small synthetic example input files for every script.
